...
export default function StatsPanel({ stats, balance }) {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4">
      <h3 className="text-md font-bold text-yellow-500 mb-3">📊 ESTATÍSTICAS</h3>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-yellow-900/30 rounded-lg p-3 text-center">
          ...
          <div className="text-xs">{total ? ((stats.banker / total) * 100).toFixed(1) : 0}%</div>
        </div>
        <div className="bg-green-900/30 rounded-lg p-3 text-center">
          ...
          <div className="text-xs">{total ? ((stats.player / total) * 100).toFixed(1) : 0}%</div>
        </div>
        <div className="bg-gray-700 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-gray-400">{stats.tie}</div>
          <div className="text-xs text-gray-400">🤝 Tie</div>
          <div className="text-xs">{total ? ((stats.tie / total) * 100).toFixed(1) : 0}%</div>
        </div>
      </div>
